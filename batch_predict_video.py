            ...
            with tqdm(total=int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), desc=f"{current_count}/{total_output_videos} Processing {output_video.name}", unit='frames') as progress_bar:
                while ret:
                    ...
                    for result in results.boxes.data.tolist():
                        ...
                        if score > threshold:
                            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0) if class_id == 1 else (0, 0, 255), 1)
                            cv2.putText(frame, results.names[int(class_id)].upper(), (int(x1), int(y1 - 10)),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0) if class_id == 1 else (0, 0, 255), 1, cv2.LINE_AA)
